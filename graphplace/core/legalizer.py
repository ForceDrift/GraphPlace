...
class Legalizer:
    ...
    def legalize(self):
        ...
        if is_legal:
            ...
            is_legal, msg = self.check_legality()
            print(
                "Legalization completed via rescue pack."
                if is_legal
                else f"Final Error: {msg}"
            )
    ...
    def check_legality(self) -> Tuple[bool, str]:
        ...
        no_overlap = (
            (xi + wi / 2 <= xj - wj / 2 + 0.01)
            | (xj + wj / 2 <= xi - wi / 2 + 0.01)
            | (yi + hi / 2 <= yj - hj / 2 + 0.01)
            | (yj + hj / 2 <= yi - hi / 2 + 0.01)
        )
        ...
        oob = (
            (self.x < self.widths / 2 - 0.1)
            | (self.x > self.benchmark.canvas_width - self.widths / 2 + 0.1)
            | (self.y < self.heights / 2 - 0.1)
            | (self.y > self.benchmark.canvas_height - self.heights / 2 + 0.1)
        )
        if oob.any():
